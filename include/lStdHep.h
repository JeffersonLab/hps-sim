...
//
        enum {
            MAJOR = 2, MINOR = 0, DAY = 23, MONTH = 1, YEAR = 2006
        };
    ...
    public:
        ...
        static const char *getText(void) {
            static char buff[80];
            sprintf(buff, "lStdHep version %d.%d (%02d.%02d.%d) by W.G.J. Langeveld, SLAC", MAJOR, MINOR, DAY, MONTH,
                    YEAR);
            return (buff);
        }
...
//
        ...
        double X(int i) const {
            return (event.vhep[i * 4 + 0]);
        }

        double Y(int i) const {
            return (event.vhep[i * 4 + 1]);
        }

        double Z(int i) const {
            return (event.vhep[i * 4 + 2]);
        }

        double T(int i) const {
            return (event.vhep[i * 4 + 3]);
        }

        double Px(int i) const {
            return (event.phep[i * 5 + 0]);
        }

        double Py(int i) const {
            return (event.phep[i * 5 + 1]);
        }

        double Pz(int i) const {
            return (event.phep[i * 5 + 2]);
        }

        double E(int i) const {
            return (event.phep[i * 5 + 3]);
        }

        double M(int i) const {
            return (event.phep[i * 5 + 4]);
        }

        long pid(int i) const {
            return (event.idhep[i]);
        }

        long status(int i) const {
            return (event.isthep[i]);
        }

        long mother1(int i) const {
            return (event.jmohep[i + i + 0]);
        }

        long mother2(int i) const {
            return (event.jmohep[i + i + 1]);
        }

        long daughter1(int i) const {
            return (event.jdahep[i + i + 0]);
        }

        long daughter2(int i) const {
            return (event.jdahep[i + i + 1]);
        }
        ...
        double alphaQED(void) const {
            return (event.alphaqed);
        }

        double alphaQCD(void) const {
            return (event.alphaqcd);
        }

        double scale(int i, int j) const {
            return (event.scale[i * 10 + j]);
        }

        double spinX(int i) const {
            return (event.spin[i * 3 + 0]);
        }

        double spinY(int i) const {
            return (event.spin[i * 3 + 1]);
        }

        double spinZ(int i) const {
            return (event.spin[i * 3 + 2]);
        }

        long colorflow(int i, int j) const {
            return (event.colorflow[i * 2 + j]);
        }
